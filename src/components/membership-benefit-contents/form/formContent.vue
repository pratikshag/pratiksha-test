...
export default {
  ...
  methods: {
    formReset() {
      ...
      this.benefit.url = '';
      this.benefit.is_active = true;
      this.is_loading = false;
    },

    fetchBenefit() {
      this.benefit_id = this.$route.params.id;
      this.formReset();

      if (this.benefit_id) {
        this.is_loading = true;
        return Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-benefit-contents', {
          params: { filter: { _id: this.benefit_id } },
        })
          .then((response) => {
            this.is_loading = false;
            ...
            if (response.status == 200 && response.data.data && response.data.data.length > 0) {
              ...
              this.benefit.id = benefit._id;
              this.benefit.title = benefit.title;
              this.benefit.description = benefit.description;
              this.benefit.image = benefit.image;
              this.benefit.url = benefit.url;
              this.benefit.is_active = benefit.is_active;
            }
          })
          .catch((err) => {
            console.log(err);
    ...
    saveBenefit(scope) {
      ...
      self.$validator.validateAll(scope).then((result) => {
        if (result) {
          if (self.benefit_id) {
            Axios.put(
              Config.default.MS_SOCO_ADMIN_API_URL + '/membership-benefit-content/' + self.benefit_id,
              self.benefit
            )
              .then((response) => {
                console.log(response);
                if (response.status === 200) {
                  self.$router.push('/membership/benefit/all');
                }
              })
              .catch((err) => {
                self.$Progress.fail();
            ...
            Axios.post(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-benefit-contents', self.benefit)
              .then((response) => {
                console.log(response);
                if (response.status === 200) {
                  self.$router.push('/membership/benefit/all');
