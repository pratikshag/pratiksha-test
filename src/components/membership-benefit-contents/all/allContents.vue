...
export default {
  name: 'membership-benefit-content',
  components: {},
  ...
  methods: {
    getCount: function (filter) {
      ...
      Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-benefit-contents/count', {
        params: {
    ...
    fetchData: function (filter) {
      ...
      Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-benefit-contents', {
        params: {
          filter: this.criteria,
    ...
    filterData() {
      if (this.filter.title) {
        this.criteria.title = { $regex: this.filter.title, $options: 'i' };
      } else {
        delete this.criteria.title;
      }
    ...
    trash: function (id) {
      ...
      self.$dialog
        ...
        .then(function (dialog) {
          ...
          Axios.delete(Config.default.MS_SOCO_ADMIN_API_URL + `/membership-benefit-content/${id}`).then((response) => {
            self.is_loading = true;
            if (response.status == 200) {
              self.fetchData();
              self.getCount();
