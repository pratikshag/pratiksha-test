...
export default {
  ...
  methods: {
    getCount: function (filter) {
      ...
      Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-level-contents/count', {
        params: {
    ...
    fetchData: function (filter) {
      ...
      Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-level-contents', {
        params: {
          filter: this.criteria,
    ...
    trash: function (id) {
      ...
      self.$dialog
        ...
        .then(function (dialog) {
          ...
          Axios.delete(Config.default.MS_SOCO_ADMIN_API_URL + `/membership-level-content/${id}`).then((response) => {
            self.is_loading = true;
            if (response.status == 200) {
              self.fetchData();
              self.getCount();
