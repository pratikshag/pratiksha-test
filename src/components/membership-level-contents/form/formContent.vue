  ...
  methods: {
    ...
    fetchLevel() {
      ...
      if (this.level_id) {
        ...
        return Axios.get(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-level-contents', {
          ...
          .then((response) => {
            ...
            if (response.status == 200 && response.data.data && response.data.data.length > 0) {
              ...
              this.level.image = level.image;
              this.level.is_active = level.is_active;
            }
          })
          .catch((err) => {
            console.log(err);
    ...
    saveLevel(scope) {
      ...
      self.$validator.validateAll(scope).then((result) => {
        if (result) {
          if (self.level_id) {
            Axios.put(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-level-content/' + self.level_id, self.level)
              .then((response) => {
                console.log(response);
                if (response.status === 200) {
                  self.$router.push('/membership/level/all');
                }
              })
              .catch((err) => {
                self.$Progress.fail();
            ...
            Axios.post(Config.default.MS_SOCO_ADMIN_API_URL + '/membership-level-contents', self.level)
              .then((response) => {
                console.log(response);
                if (response.status === 200) {
                  self.$router.push('/membership/level/all');
