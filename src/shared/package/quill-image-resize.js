...
export class ImageResize {
	...
	constructor(quill, options = {}) {
		...
		this.showResizers = this.showResizers.bind(this);
		this.showSizeDisplay = this.showSizeDisplay.bind(this);
		this.wrapper = document.createElement('div');
		this.wrapper.setAttribute('id', 'wrapper-resizing-vue2');
	...
	hideResizers() {
		...
		this.setCursor('');
		// remove boxes
		this.boxes.forEach(box => document.getElementById('wrapper-resizing-vue2').removeChild(box));
		// release memory
	...
	addBox(cursor) {
		...
		document.getElementById('wrapper-resizing-vue2').appendChild(box);
	...
	handleTouchStart(evt) {
		this.dragBox = evt.target;
		// note starting touchstart position
		this.dragStartX = evt.touches[0].clientX;
		// store the width before drag
		this.preDragWidth = this.img.width || this.img.naturalWidth;
		// listen movement drag touch
	...
	handleDrag(evt) {
		...
		var resizingWidthImgRight = this.preDragWidth + evt.clientX - this.dragStartX;
		var resizingWidthImgLeft = this.preDragWidth - evt.clientX - this.dragStartX;
		...
		if (this.dragBox == this.boxes[0] || this.dragBox == this.boxes[3]) {
			...
			if(resizingWidthImgLeft > 50) {
				this.img.width = resizingWidthImgLeft;
			}
			else {
				this.img.width = 50;
			}
			...
			//this.img.width = Math.round(this.preDragWidth + evt.clientX - this.dragStartX);
			if(resizingWidthImgRight > 50) {
				this.img.width = resizingWidthImgRight;
			}
			else {
				this.img.width = 50;
			}
	...
	handleTouchDrag(evt) {
		...
		const minWidthImg = 50;
		var resizingWidthImgRight = this.preDragWidth + evt.changedTouches[0].clientX - this.dragStartX;
		var resizingWidthImgLeft = this.preDragWidth - evt.changedTouches[0] - this.dragStartX;
	...
	setCursor(value) {
		[
			document.getElementById('wrapper-resizing-vue2'),
			this.img,
			this.quill.root
	...
	showSizeDisplay() {
		...
		document.getElementById('wrapper-resizing-vue2').appendChild(this.display);
	}

	hideSizeDisplay() {
		document.getElementById('wrapper-resizing-vue2').removeChild(this.display);
